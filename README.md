...
1. Explain how to build stateful class components.
   ...
   render() {
   <Random fill={this.state.fill} />
   }
   }
2. Describe the different phases of the component lifecycle.
   Simple version: constructor(), render(), componentDidMount().
   constructor() - first thing to be called, therefore call with props, this initiates the parents constructor method and allows the component to inherit methods from it parent React.Component.
   render() - this method actually renders the HTML in the DOM
   componentDidMount() - allows for side effects and event listeners - this can include things like fetching inital data (you'd use componentDidUpdate() to get new data if state or props change)
3. Demonstrate an understanding of class component lifecycle methods.
   see above
4. Define stateful logic.
   Stateful logic is any code we create that uses state.
5. Describe how to test a React component with React Testing Library.
   Arrange - initialise objects and sets the value of the data to be passed in to test
   Act - invoke the method under test with params
   Assert - makes sure the method under test responds as expected
